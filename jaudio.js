...
(function($){
  ...
  Plugin.prototype = {
    ...
    getAudioMinutes: function(string)
    {
      ...
      (string >= 60) ? string = self.addZero( Math.floor(string - 60), 2) : string = string;
      (string < 60) ? string = string : string = "00";

      return string;
    },

    getAudioHours: function(string)
    {
      var self    = this,
          string  = string / 60 / 60;
          string  = self.addZero( Math.floor(string), 2 );
    ...
    renderPlaylist: function()
    {
      ...
      $.each(self.settings.playlist, function(i, a)
      {
        ...
        template += "<div class='jAudio--playlist-meta-track-name'>" + trackName + "</div>";
        template += "<div class='jAudio--playlist-meta-track-artist'>" + trackArtist + "</div>";
        template += "</div>";
    ...
    updateElapsedTime: function()
    {
      var self      = this,
          ...
          hours     = self.getAudioHours(time),
          minutes   = self.getAudioMinutes(time),
          seconds   = self.getAudioSeconds(time),
          audioTime = hours + ":" + minutes + ":" + seconds;

      self.$domElapsedTime.text( audioTime );
      
        time2      = self.domAudio.duration - time,
        hours     = self.getAudioHours(time2),
        minutes   = self.getAudioMinutes(time2),
        seconds   = self.getAudioSeconds(time2),
        audioTime = hours + ":" + minutes + ":" + seconds;
